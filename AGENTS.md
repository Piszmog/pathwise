...
- **Generate**: `go tool templ generate -path ./internal/ui/components && go tool sqlc generate`
- **E2E tests**: `go test -tags=e2e ./internal/ui/e2e/...`
- **Integration tests**: `go test ./internal/mcp/tool/... -tags=integration`
...
- **Templates**: Use templ components in internal/ui/components/, call with `h.html(ctx, w, status, component)`
...
- **E2E**: Playwright tests in internal/ui/e2e/, use `//go:build e2e` tag  
- **Integration**: MCP tests with `//go:build integration` tag
- **Run E2E**: Requires Playwright setup, tests web interface end-to-end

## Templ & HTMX
- **Templ**: Components in internal/ui/components/*.templ, use `@componentName()` syntax, generate with `go tool templ generate`
