...
func New(logger *slog.Logger, database db.Database) http.Handler {
	...
	protected.HandleFunc(http.MethodGet+" /job-listings", h.GetJobListings)
	protected.HandleFunc(http.MethodGet+" /job-listings/{id}", h.GetJobListingDetails)
	...
	protected.HandleFunc(http.MethodGet+" /analytics", h.Analytics)
	protected.HandleFunc(http.MethodGet+" /analytics/graph", h.AnalyticsGraph)
